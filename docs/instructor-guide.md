...
| 0:15 - 0:45 | Step 1〜2 | テンプレート構造、Parameters、!Ref、変更セットを解説しながら進行 |
...
**変更セットの説明**:
- Step 2 で初めて変更セットを使うので、ここで丁寧に手順を見せる
- 「変更セットは本番運用では必須の手順。いきなり適用せず、まずプレビューする」を強調
- 変更セットの Add / Modify / Remove の意味を説明
- 「変更セット作成 → 内容確認 → 実行」の 3 ステップを以降全てのスタック更新で統一する

**デプロイ後に見せるもの**:
- 変更セットの「変更」セクションで、サブネット × 2 が Add になっていることを確認
- 実行後、イベントタブで「VPC は変更なし、サブネットだけ新規作成」を確認 → **スタック更新は差分だけ適用される**
